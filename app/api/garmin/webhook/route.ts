...
import { bodyHasHealthKeys } from '@/lib/garmin-events/process-health-webhook';
...
async function processWebhookSafely(rawText: string): Promise<void> {
  try {
    ...
    if (bodyHasHealthKeys(body)) {
      console.warn(
        '⚠️ Health payload received on activity webhook — point Sleeps/Dailies to /api/garmin/health-webhook in Garmin portal'
      );
    }

...
export async function GET(
  ...
  const item = await prisma.intervals_config.findUnique({
    where: { id },
    include: {
      ...includeBlock,
...
export async function PATCH(
  ...
  if (authErr) return authErr;
  const { id } = await params;
  const existing = await prisma.intervals_config.findUnique({ where: { id } });
  if (!existing) {
    return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  }
  ...
  const item = await prisma.intervals_config.update({
    where: { id },
    data,

...
export default function RunClubLeaderOverviewPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (user) => {
      if (!user) {
        router.replace(`/signup?redirect=/leader/runclub/${slug}`);
        return;
      }
      load();
    });
  ...
  const base = `/leader/runclub/${slug}`;
  ...
  const runsStatus = setup?.hasSeries && setup?.hasUpcomingRuns
  ...
  return (
    <RunClubLeaderShell clubName={club.name} clubSlug={slug} active="overview">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-600 mt-2 max-w-2xl">
          Start with club metadata and weekly runs. Announcements and invites come after the basics
          are in place.
        </p>
      </div>
      ...
      {setup && !setup.readyForMembers ? (
        <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          ...
          {setup.metaMissing.length > 0
            ? `Still missing: ${setup.metaMissing.join(', ')}.`
            : null}
      ...
      <div className="space-y-4 mb-10">
        ...
        <ManagerWizardCard
          ...
          statusLabel={
            announcementsSummary.count > 0
              ? `${announcementsSummary.count} posted`
              : 'None yet'
          }
          statusTone={announcementsSummary.count > 0 ? 'neutral' : 'neutral'}
          detail={
            announcementsSummary.latest
              ? `Latest: ${announcementsSummary.latest.title ?? announcementsSummary.latest.body.slice(0, 60)}…`
              : 'Share news after meta and runs are set.'
          }
        />
      </div>

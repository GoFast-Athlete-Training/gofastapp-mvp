...
export default function RunClubLeaderAnnouncementsPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      if (!user) {
        router.replace(`/signup?redirect=/leader/runclub/${slug}/announcements`);
        return;
      }
      try {
        await load();
  ...
  return (
    <RunClubLeaderShell clubName={clubName} clubSlug={slug} active="announcements">
      <div className="mb-6 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
        ...
        <p className="text-sm text-gray-600 mt-1">
          Announcements help after your club meta and runs are set. Members see these in the club
          experience.
        </p>
      </div>
      <form

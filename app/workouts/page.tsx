...
function LastRunPanel({ workout }: { workout: LastLoggedWorkout }) {
  ...
  if (distMi) statsParts.push(distMi);
  if (actual != null) statsParts.push(`${formatSecPerMileDisplay(actual)} avg`);
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <p className="text-sm text-gray-900">
        ...
        {day ? <span className="text-gray-600"> · {day}</span> : null}
      </p>
      {workout.workoutType ? (
        <span className="mt-2 inline-block rounded-md bg-orange-50 px-2 py-0.5 text-xs font-medium text-orange-800 capitalize">
          {String(workout.workoutType).toLowerCase()}
        </span>
      ) : null}
      {statsParts.length > 0 ? (
        <p className="mt-3 text-sm text-gray-800">{statsParts.join(" · ")}</p>
      ) : null}
      {versus != null ? (
        ...
        <p className="mt-2 text-sm text-gray-700">{band}</p>
      ) : null}
      {workout.hrDeltaBpm != null ? (
        <p className="mt-2 text-sm text-gray-700">
          HR vs zone:{" "}
          {workout.hrDeltaBpm > 0
            ? `${workout.hrDeltaBpm} bpm under target midpoint`
            : workout.hrDeltaBpm < 0
              ? `${Math.abs(workout.hrDeltaBpm)} bpm above target midpoint`
              : "on midpoint"}
        </p>
      ) : null}
    </section>
...
function GoTrainToday() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [standaloneRes, listRes, lastRunRes] = await Promise.all([
        fetch(
  ...
  return (
    <div className="space-y-6">
      ...
      {authReady && loading ? <LastRunPanelSkeleton /> : null}
      {authReady && !loading && lastRun ? <LastRunPanel workout={lastRun} /> : null}
    </div>
  );
}

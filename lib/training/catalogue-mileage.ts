/**
 * Stub estimated total miles for a catalogue prescription (Intervals / Tempo primarily).
 * Replaces legacy qualityFraction sizing — full fidelity can expand parsing of segmentPaceDist + recoveries later.
 */

import type { workout_catalogue, WorkoutType } from "@prisma/client";

/** Fields needed from DB rows for stub estimation */
...
export function estimateCatalogueWorkoutMiles(
  ...
  if (Number.isFinite(wbMi) && wbMi > 0) {
    return Math.max(
      3,
      Math.round((warmup + cooldown + wbMi) * 10) / 10
    );
  }
  ...
  if (
    ...
    return Math.max(
      3,
      Math.round((warmup + cooldown + workMi) * 10) / 10
    );
  }

...
function longRunMaxFromPeakMiles(peakMiles: number | undefined | null): number {
  if (peakMiles != null && Number.isFinite(peakMiles) && Number(peakMiles) > 0) {
    return Math.max(8, Math.round(Number(peakMiles) * 0.28 * 10) / 10);
  }
  return 22;
}
 ...
 * `peakMiles` anchors max distance; internal taper still uses the last 4-week block in the LRE.
 */
export function longRunConfigFromPlanGen(
  totalWeeks: number,
  cfg?: { minLongMiles?: number; peakMiles?: number | null }
  ...
  return {
    ...
    step: 2,
    longRunMin: minL,
    longRunMax: longRunMaxFromPeakMiles(cfg?.peakMiles),
  };
}

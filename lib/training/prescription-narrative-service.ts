...
/**
 * Ensures planned_workouts.prescriptionNarrative exists for catalogue-backed plan days.
 */
export async function ensurePlannedWorkoutPrescriptionNarrative(params: {
  plannedWorkoutId: string;
  athleteId: string;
}): Promise<void> {
  const apiKey = process.env.OPENAI_API_KEY?.trim();
  if (!apiKey) {
    console.warn("ensurePlannedWorkoutPrescriptionNarrative: OPENAI_API_KEY missing");
    return;
  }
  ...
  if (!planned?.workout_catalogue?.name || SKIP_TYPES.has(planned.workoutType)) {
    return;
  }
  if (planned.prescriptionNarrative?.trim()) {
    return;
  }
  ...
  const payload = {
    ...
    workout: {
      ...
      planName: planned.training_plans?.name ?? null,
      planTotalWeeks: planned.training_plans?.totalWeeks ?? null,
      scheduledDistanceMeters: planned.estimatedDistanceInMeters,
      structuredSteps: planned.segments.map((s) => ({
        title: s.title,
        durationType: s.durationType,
        durationValue: s.durationValue,
      })),
    ...
    catalogue: catalogueContext,
    paceTargetLine: null,
    planSchedule: planned.training_plans?.planSchedule ?? null,
  };
...
If structuredSteps is non-empty, weave in one concrete execution cue (warmup discipline, where to push, what to hold back) without Garmin jargon.`;
  ...
  try {
    ...
    if (!res.ok) {
      const errText = await res.text();
      console.error("planned prescription narrative OpenAI HTTP", res.status, errText.slice(0, 500));
      return;
    }
    const data = (await res.json()) as {
      choices?: Array<{ message?: { content?: string } }>;
    ...
    console.error("planned prescription narrative fetch:", e);
    return;
  }

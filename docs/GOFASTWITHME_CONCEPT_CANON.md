...
2. Athlete community (GoFast With {Name} Community)
   ...
   - **P0 surfaces (RunCrew / Race Hub pattern):** identity header + hub tabs — Journey (lead: weekly Updates, Training for, next run, join-me plan, tips), Runs, People, Chatter (last). Deep links (`#plan`, `#updates`, etc.) still map into those tabs.
